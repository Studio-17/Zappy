...
{
    char *request_command = ai_get_generic_request(zappy->server->socket_descriptor->socket_descriptor, zappy, player_index);

    if (!request_command)
        return false;
    for (int index = 0; index < NB_COMMANDS_AI; index += 1) {

        if (strncmp(request_command, ai_request_to_handle[index].request, strlen(ai_request_to_handle[index].request)) == 0) {
